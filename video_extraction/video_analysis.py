...
- Writes TWO outputs per file:
    1) Raw Azure response (unchanged) under video_analysis_result/raw_*.json
    2) Normalized JSON whose fields conform to custom_schema.json under
       video_analysis_result/normalized_*.json
...
# ----------------------- Configuration -----------------------
file_urls = [
    "https://stanildwa097846267954041.blob.core.windows.net/contentunderstanding/video-analysis/sample_video_with_narration.mp4",
    #"https://stanildwa097846267954041.blob.core.windows.net/contentunderstanding/video-analysis/sample_video_no_narration.mp4"
]
...
ANALYZER_ID = os.getenv("AZURE_CONTENT_UNDERSTANDING_ANALYZER_ID", "prebuilt-videoAnalyzer")

OUTPUT_DIR = "video_analysis_result"
...
def run_video_analysis(fileName: str = None):
    ...
    for file_url in file_urls:
        ...
        # Write raw
        raw_path = Path(f"{OUTPUT_DIR}/raw_{Path(file_url).name}.json")
        with open(raw_path, "w", encoding="utf-8") as f:
            json.dump(result, f, indent=2)

        # Normalize
        normalized = normalize_to_custom_schema(result)
        norm_path = Path(f"{OUTPUT_DIR}/normalized_{Path(file_url).stem}.json")
